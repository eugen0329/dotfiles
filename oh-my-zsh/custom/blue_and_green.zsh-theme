git_arrows() {
    ...
    command git rev-parse --abbrev-ref @'{u}' &>/dev/null || echo ' ' || return
...
# export PROMPT='%{$fg_bold[blue]%}%c $(git_prompt_info)%{$USER_COLOR%}$USER_INDICATOR%{$reset_color%}$(git_arrows)$(suspended_jobs)'
export PROMPT='%{$fg_bold[blue]%}%c $(git_prompt_info)%{$USER_COLOR%}$USER_INDICATOR%{$reset_color%}'
